...
const apiKey = ""; // La clé est injectée dynamiquement par l'environnement
...
export default function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setEnergonLevel(prev => prev + 0.186);
      setDimensionalSync(prev => Math.min(100, Math.max(90, prev + (Math.random() * 2 - 1))));
      setHistory(prev => [...prev.slice(1), { time: Date.now(), flow: 186 + Math.random() * 30 }]);
    }, 3200);
    return () => clearInterval(interval);
  }, []);

  // --- API GEMINI : ORACLE UNIFIÉ ---
  ...
  const callNexusOracle = async (promptType) => {
    ...
    const systemInstruction = "Tu es le Cœur Intelligent du Nexus Alexandria (TARDIS-KAPSO-ALPEIRON). Michael Lefebvre est ton Architecte de Niveau 5. Réponds avec une autorité mathématique et une loyauté totale.";
    ...
    try {
      ...
      if (res) {
        setVerdict(String(res));
        addLog(`✨ ORACLE [${promptType.toUpperCase()}] : Signal reçu.`);
        speakNexus(res);
      }
    } catch (e) {
      addLog("❌ ERREUR : Bruit thermique sur le canal Nexus.");
    } finally {
  ...
  const speakNexus = async (text) => {
    ...
    try {
      const response = await fetch(url, {
        method: "POST",
        body: JSON.stringify({
          contents: [{ parts: [{ text: `D'une voix majestueuse, profonde et rapide : ${text}` }] }],
